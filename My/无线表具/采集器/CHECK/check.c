...
u8 GetWorkingMode(void)
{
    u8 EXTWrkMode;
    u8 InterWrkMode;
    SPI_Flash_Read(EXT_CJQ_PARAMETER_PAGEADDR+16,&EXTWrkMode,1);//读取外部flash中工作模式位
    ReadFlash_Byte(CJQ_PARAMETER_PAGEADDR+16,&InterWrkMode,1 );//读取内部flash中工作模式位
    if (EXTWrkMode==InterWrkMode)
    {
        return InterWrkMode;
    }
    else
    {
        SPI_Flash_Write(EXT_CJQ_PARAMETER_PAGEADDR+16,&InterWrkMode,1);//内部flash模式写入外部flash
    }
    return InterWrkMode;
}
